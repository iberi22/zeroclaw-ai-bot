...
## Agentic Benchmark System

ZeroClaw includes an agent benchmark runner to continuously improve behavior/configuration
in an isolated profile.

```bash
# Baseline benchmark
python scripts/agent_benchmark.py \
  --exe C:/Users/<user>/.cargo/target_global/release/zeroclaw.exe \
  --tasks benchmarks/agent_tasks.json \
  --profile-root ~/.zeroclaw-benchmark

# Multi-loop agentic cycle (benchmark -> tune -> rerun)
python scripts/agent_benchmark.py \
  --exe C:/Users/<user>/.cargo/target_global/release/zeroclaw.exe \
  --tasks benchmarks/agent_tasks.json \
  --profile-root ~/.zeroclaw-benchmark \
...
- `~/.zeroclaw-benchmark/benchmarks/runs/*.json`
- `~/.zeroclaw-benchmark/benchmarks/runs/*.md`
- `~/.zeroclaw-benchmark/benchmarks/runs/*.self_analysis.md`

To clone OpenClaw behavior/context into an isolated workspace (without touching OpenClaw source):

```bash
# Preview only
python scripts/openclaw_workspace_clone.py --dry-run

# Perform clone into isolated bridge profile
python scripts/openclaw_workspace_clone.py \
  --target-root ~/.zeroclaw-openclaw-bridge
```

This clone flow keeps source immutable and lets ZeroClaw mirror bootstrap context
(`AGENTS.md`, `SOUL.md`, `TOOLS.md`, `IDENTITY.md`, `USER.md`, `MEMORY.md`, skills, etc.)
inside its own workspace for behavior parity with OpenClaw-style bots.

Windows continuous loop (scheduled):

```powershell
# Install scheduled task (every 6 hours)
powershell -ExecutionPolicy Bypass -File scripts/install_agentic_benchmark_task.ps1 -RepeatMinutes 360

# Run once manually
powershell -ExecutionPolicy Bypass -File scripts/run_agentic_loop.ps1
```

Quality gate on latest run summary:

```bash
python scripts/benchmark_gate.py \
  --summary ~/.zeroclaw-openclaw-bridge/benchmarks/runs/<run_id>.summary.json \
  --min-pass-rate 70 \
  --min-score-ratio 0.70
```

Notifications (Telegram/Discord) from loop script:

```powershell
# Telegram
setx ZEROCLAW_NOTIFY_TELEGRAM_BOT_TOKEN "<bot_token>"
setx ZEROCLAW_NOTIFY_TELEGRAM_CHAT_ID "<chat_id>"

# Discord
setx ZEROCLAW_NOTIFY_DISCORD_WEBHOOK_URL "<webhook_url>"

# Run loop (notifies on failure by default)
powershell -ExecutionPolicy Bypass -File scripts/run_agentic_loop.ps1

# Optional: notify also on pass
powershell -ExecutionPolicy Bypass -File scripts/run_agentic_loop.ps1 -NotifyOnPass
```
