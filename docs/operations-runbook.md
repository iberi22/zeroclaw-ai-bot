...
## Continuous Agent Benchmark Loop

Use this loop to improve agent configuration continuously with measurable evidence.

1. Run benchmark suite in an isolated profile:
...
2. Run benchmark + self-analysis (agent proposes config improvements):
...
- `~/.zeroclaw-benchmark/benchmarks/runs/<timestamp>.json`
- `~/.zeroclaw-benchmark/benchmarks/runs/<timestamp>.md`
- `~/.zeroclaw-benchmark/benchmarks/runs/<timestamp>.self_analysis.md` (when enabled)

4. Apply one config change at a time in benchmark profile, rerun, then compare scores.

5. Promote only validated changes into production profile after a stable benchmark gain.
