...
async fn main() -> Result<()> {
    ...
    if let Some(crash) = zeroclaw::resilience::consume_crash(&config.workspace_dir) {
        zeroclaw::resilience::report_task(
            "Recovered from Crash",
            &format!("The bot was restarted after a crash: {}", crash.error),
            &config.workspace_dir,
        );
        info!("Restored from previous crash: {}", crash.error);
    }

    // Set up panic hook for resilience
    let workspace_for_panic = config.workspace_dir.clone();
    std::panic::set_hook(Box::new(move |info| {
        ...
        let location = info
            ...
            .unwrap_or_default();
        let error_msg = format!("PANIC: {message}{location}");

        zeroclaw::resilience::record_crash(&error_msg, &workspace_for_panic);
        eprintln!("{error_msg}");
    }));
