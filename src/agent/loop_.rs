...
static INLINE_TOOL_KV_REGEX: LazyLock<Regex> =
...
fn load_skills_with_sync(config: &Config) -> Vec<crate::skills::Skill> {
    let mut skills = crate::skills::load_skills(&config.workspace_dir);
    if config.integration.openclaw_sync {
        if let Some(openclaw_workspace) = crate::util::resolve_openclaw_workspace() {
            if openclaw_workspace != config.workspace_dir {
                skills.extend(crate::skills::load_skills(&openclaw_workspace));
            }
        }
    }
    skills
...
fn parse_inline_tool_kv_arguments(input: &str) -> Option<serde_json::Value> {
    let mut args = serde_json::Map::new();
    for captures in INLINE_TOOL_KV_REGEX.captures_iter(input) {
...
fn parse_glm_style_tool_calls(text: &str) -> Vec<(String, serde_json::Value, Option<String>)> {
    ...
    for line in text.lines() {
        ...
        if let Some(open_pos) = line.find('(') {
            if open_pos > 0 && line.ends_with(')') {
                let tool_part = line[..open_pos].trim();
                let args_part = line[open_pos + 1..line.len() - 1].trim();
                if tool_part.chars().all(|c| c.is_alphanumeric() || c == '_') {
                    if args_part.contains("=\"") {
                        if let Some(arguments) = parse_inline_tool_kv_arguments(args_part) {
        ...
        if line.contains("=\"") {
            ...
            if let Some(tool_part) = parts.next() {
                let rest = parts.next().unwrap_or("").trim();
                if !rest.is_empty() && tool_part.chars().all(|c| c.is_alphanumeric() || c == '_') {
                    if let Some(arguments) = parse_inline_tool_kv_arguments(rest) {
                        let tool_name = map_glm_tool_alias(tool_part);
                        calls.push((tool_name.to_string(), arguments, Some(line.to_string())));
                        continue;
                    }
        ...
        // STRICTOR: Must start with a known tool prefix or look very much like a command.
        if let Some(pos) = line.find('/') {
            ...
            if tool_part.chars().all(|c| c.is_alphanumeric() || c == '_') {
                ...
                if let Some(gt_pos) = rest.find('>') {
                    ...
                    // Only parse if param_name looks like a valid argument key
...
pub(crate) async fn run_tool_call_loop(
    ...
    let mut last_tool_calls_hash = String::new();
    ...
    for iteration in 0..max_iterations {
        ...
        // Circular tool-call detection: If the model repeats the exact same tool calls
        // from the previous iteration, it's likely stuck.
        let current_calls_hash = format!("{:?}", tool_calls);
        if current_calls_hash == last_tool_calls_hash && iteration > 0 {
            tracing::warn!(
                "Circular tool call detected in iteration {}. Aborting loop.",
                iteration
            );
            history.push(ChatMessage::assistant(response_text.clone()));
            // Return what we have so far instead of bailing with an error.
            return Ok(display_text);
        }
        last_tool_calls_hash = current_calls_hash;
...
pub async fn run(
    ...
    let skills = load_skills_with_sync(&config);
    ...
    let mut system_prompt = crate::channels::build_system_prompt_with_integration(
        &config.workspace_dir,
        model_name,
        &tool_descs,
        &skills,
...
Done."#;
    ...
    #[test]
    fn parse_glm_style_rejects_plain_url() {
        // Plain URLs should no longer be auto-parsed without tool prefix
...
browser_open/url>https://example.com"#;
    ...
    #[test]
    fn parse_glm_style_handles_send_message_inline_attributes() {
        let response = r#"send_message channel="user_channel" message="Hello!""#;
        let calls = parse_glm_style_tool_calls(response);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].0, "message_send");
        assert_eq!(calls[0].1["channel"], "user_channel");
        assert_eq!(calls[0].1["message"], "Hello!");
    }

    #[test]
    fn parse_glm_style_handles_function_style_arguments() {
        let response = r#"message_send(channel="general", message="test")"#;
        let calls = parse_glm_style_tool_calls(response);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].0, "message_send");
        assert_eq!(calls[0].1["channel"], "general");
        assert_eq!(calls[0].1["message"], "test");
    }
    ...
    #[tokio::test]
    async fn test_circular_tool_call_detection() {
        ...
        impl MockProvider {
            ...
            fn add_response(&self, resp: ChatResponse) {
        ...
        // Return the same tool call twice
        provider.add_response(ChatResponse {
            text: Some("looping".into()),
        ...
        let mut history = vec![ChatMessage::user("run ls")];
        ...
        assert_eq!(final_text, "looping again");
        // Loop should have broken before hitting 10 iterations
