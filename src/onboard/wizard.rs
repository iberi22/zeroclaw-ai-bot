...
pub fn run_wizard() -> Result<Config> {
    ...
    let (workspace_dir, config_path, openclaw_sync) = setup_workspace()?;
    ...
    let (memory_config, shared_memory) = setup_memory(openclaw_sync)?;
    ...
    let config = Config {
        ...
        agent: persistent_agent_defaults(),
        ...
        integration: crate::config::schema::IntegrationConfig {
            openclaw_sync,
            shared_memory,
        },
...
#[derive(Debug, Clone)]
struct OpenClawBootstrapConfig {
...
fn persistent_agent_defaults() -> crate::config::schema::AgentConfig {
    crate::config::schema::AgentConfig {
        // Reduce "loss of context" by keeping longer in-session history.
...
pub fn run_quick_setup(
    ...
    let mut workspace_dir = zeroclaw_dir.join("workspace");
    let config_path = zeroclaw_dir.join("config.toml");

    let mut openclaw_sync = false;
    let mut shared_memory = false;
    let mut imported_openclaw = false;

    let openclaw_bootstrap = maybe_detect_openclaw_bootstrap();
    if let Some(ref bootstrap) = openclaw_bootstrap {
        println!(
            "  {} OpenClaw detected at {}",
            style("✓").green().bold(),
            style(bootstrap.workspace_dir.display()).green()
        );
        if std::io::stdin().is_terminal() {
            let reuse = Confirm::new()
                .with_prompt("  Reuse OpenClaw project provider/model/workspace config?")
                .default(true)
                .interact()
                .unwrap_or(false);
            if reuse {
                imported_openclaw = true;
                openclaw_sync = true;
                shared_memory = true;
                workspace_dir = bootstrap.workspace_dir.clone();
            }
        }
    }
    ...
    let provider_name = provider
        ...
        .or_else(|| {
            if imported_openclaw {
                openclaw_bootstrap.as_ref().map(|b| b.provider.clone())
            } else {
                None
            }
        })
        .unwrap_or_else(|| "openrouter".to_string());
    let model = if imported_openclaw {
        openclaw_bootstrap
            .as_ref()
            .map(|b| b.model.clone())
    ...
    let resolved_api_url = if imported_openclaw {
        openclaw_bootstrap.as_ref().and_then(|b| b.api_url.clone())
    } else {
        None
    };
    let resolved_api_key = credential_override.map(String::from).or_else(|| {
        if imported_openclaw {
            openclaw_bootstrap.as_ref().and_then(|b| b.api_key.clone())
        } else {
            None
        }
    });
    ...
    if imported_openclaw {
        println!(
            "  {} OpenClaw:   {}",
            style("✓").green().bold(),
            style("imported provider/model/workspace defaults").green()
        );
    }
    println!(
        "  {} Context:    history={} turns, tool-iters={}",
        style("✓").green().bold(),
...
fn setup_workspace() -> Result<(PathBuf, PathBuf, bool)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".zeroclaw");

    let openclaw_workspace = crate::util::resolve_openclaw_workspace();
    let mut openclaw_sync = false;

    if let Some(ref oc_ws) = openclaw_workspace {
        println!();
        println!(
            "  {} {}",
            style("OpenClaw Detected!").cyan().bold(),
            style("— we found an existing OpenClaw installation.").dim()
        );
        print_bullet(&format!(
            "Workspace found at: {}",
            style(oc_ws.display()).green()
        ));
        print_bullet(
            "You can sync your ZeroClaw bot with this workspace to share identity and skills.",
        );
        println!();

        openclaw_sync = Confirm::new()
            .with_prompt("  Sync identity and skills with OpenClaw workspace?")
            .default(true)
            .interact()?;
    }

    let (zeroclaw_dir, workspace_dir) = if openclaw_sync {
        let oc_ws = openclaw_workspace.unwrap();
        // If syncing, we still use ~/.zeroclaw for config but point workspace to OC
        (default_dir, oc_ws)
    } else {
        print_bullet(&format!(
            "Default location: {}",
        ...
        let ws_dir = z_dir.join("workspace");
        (z_dir, ws_dir)
    };
    ...
    Ok((workspace_dir, config_path, openclaw_sync))
}
...
fn setup_memory(openclaw_sync: bool) -> Result<(MemoryConfig, bool)> {
    ...
    if openclaw_sync {
        print_bullet(
            "Since you are syncing with OpenClaw, you can also access its existing memories.",
        );
        shared_memory = Confirm::new()
            .with_prompt("  Access existing OpenClaw memories (MEMORY.md)?")
            .default(true)
            .interact()?;
        println!();
    }
