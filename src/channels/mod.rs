...
pub use traits::{Channel, ChannelMessage, MessageAttachment, SendMessage};
...
use crate::providers::traits::ImageUrl;
...
const CHANNEL_HISTORY_STATE_FILE: &str = "channel_history.json";
...
struct ChannelHistoryState {
    ...
    route_overrides: HashMap<String, ChannelRouteSelection>,
}
...
fn telegram_thread_id_from_reply_target(msg: &traits::ChannelMessage) -> Option<&str> {
...
fn conversation_memory_key(msg: &traits::ChannelMessage) -> String {
    // Preserve per-topic isolation for Telegram forum topics while keeping
    // existing key shape for channels without topic/thread semantics.
    if let Some(thread_id) = telegram_thread_id_from_reply_target(msg) {
        return format!("{}_{}_{}_{}", msg.channel, thread_id, msg.sender, msg.id);
    }
...
fn conversation_history_key(msg: &traits::ChannelMessage) -> String {
    if let Some(thread_id) = telegram_thread_id_from_reply_target(msg) {
        return format!("{}_{}_{}", msg.channel, thread_id, msg.sender);
    }
...
fn set_route_selection(ctx: &ChannelRuntimeContext, sender_key: &str, next: ChannelRouteSelection) {
    ...
    drop(routes);
    persist_channel_history_state(ctx);
}
...
fn channel_history_state_path(workspace_dir: &Path) -> PathBuf {
    workspace_dir.join("state").join(CHANNEL_HISTORY_STATE_FILE)
}

fn load_channel_history_state(workspace_dir: &Path) -> ChannelHistoryState {
    let path = channel_history_state_path(workspace_dir);
    let Ok(raw) = std::fs::read_to_string(&path) else {
        return ChannelHistoryState::default();
    };
    let Ok(mut state) = serde_json::from_str::<ChannelHistoryState>(&raw) else {
        return ChannelHistoryState::default();
    };

    // Defensive cap in case the state file was manually edited.
    for turns in state.histories.values_mut() {
        if turns.len() > MAX_CHANNEL_HISTORY {
            let keep = turns.len() - MAX_CHANNEL_HISTORY;
            turns.drain(0..keep);
        }
    }
...
fn persist_channel_history_state(ctx: &ChannelRuntimeContext) {
    let histories = ctx
    ...
    let state = ChannelHistoryState {
        histories,
        route_overrides,
    };

    let path = channel_history_state_path(&ctx.workspace_dir);
    if let Some(parent) = path.parent() {
        if let Err(e) = std::fs::create_dir_all(parent) {
            tracing::warn!(
                "Failed to create channel state dir {}: {e}",
                parent.display()
            );
            return;
        }
    ...
    let Ok(serialized) = serde_json::to_string_pretty(&state) else {
        tracing::warn!("Failed to serialize channel history state");
        return;
    };
    if let Err(e) = std::fs::write(&path, serialized) {
        tracing::warn!(
            "Failed to persist channel history state {}: {e}",
            path.display()
        );
    }
}
...
async fn process_channel_message(ctx: Arc<ChannelRuntimeContext>, msg: traits::ChannelMessage) {
    ...
    let mut system_prompt = ctx.system_prompt.as_str().to_string();
    ...
    // Create user message, supporting multimodal content if attachments are present
    let user_message = if msg.attachments.is_empty() {
        ChatMessage::user(&enriched_message)
    } else {
        // Convert attachments to ImageUrls
        let images: Vec<ImageUrl> = msg
            .attachments
            .iter()
            .filter(|att| att.mime_type.starts_with("image/"))
            .map(|att| {
                let mime = att.mime_type.trim_start_matches("image/");
                let data_url = format!("data:image/{};base64,{}", mime, att.data);
                ImageUrl::from_data_url(data_url)
            })
            .collect();
        ...
        if images.is_empty() {
            ...
            ChatMessage::user_with_images(&enriched_message, images)
        }
    };
    ...
    match llm_result {
        Ok(Ok(response)) => {
            ...
            persist_channel_history_state(ctx.as_ref());
            println!(
                "  🤖 Reply ({}ms): {}",
...
pub fn build_system_prompt(
    ...
    build_system_prompt_with_integration(
        workspace_dir,
        model_name,
        tools,
        skills,
...
pub fn build_system_prompt_with_integration(
    workspace_dir: &std::path::Path,
    model_name: &str,
    tools: &[(&str, &str)],
    skills: &[crate::skills::Skill],
    identity_config: Option<&crate::config::IdentityConfig>,
    bootstrap_max_chars: Option<usize>,
    integration_config: Option<&crate::config::schema::IntegrationConfig>,
) -> String {
    use std::fmt::Write;
    let mut prompt = String::with_capacity(8192);
    let openclaw_sync = integration_config
        .map(|cfg| cfg.openclaw_sync)
        .unwrap_or(false);
    let shared_memory = integration_config
        .map(|cfg| cfg.shared_memory)
        .unwrap_or(true);
...
fn inject_workspace_file(
    ...
    if let Ok(content) = std::fs::read_to_string(&local_path) {
        if !content.trim().is_empty() {
            contents.push((local_path, content));
        }
    }

    if openclaw_sync {
        if let Some(openclaw_workspace) = crate::util::resolve_openclaw_workspace() {
            if openclaw_workspace != workspace_dir {
                let openclaw_path = openclaw_workspace.join(filename);
                if let Ok(content) = std::fs::read_to_string(&openclaw_path) {
                    if !content.trim().is_empty() {
                        contents.push((openclaw_path, content));
                    }
                }
            }
        }
    ...
    for (path, content) in contents {
        let trimmed = content.trim();
        let _ = writeln!(prompt, "*(Loaded from {})*", path.display());
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if let Some(ref tg) = config.channels_config.telegram {
        ...
        for extra in &tg.extra_bots {
            channels.push((
                "Telegram (extra)",
                Arc::new(
                    TelegramChannel::new(
                        extra.bot_token.clone(),
                        extra.allowed_users.clone(),
                        extra.mention_only,
                    )
                    .with_streaming(tg.stream_mode, tg.draft_update_interval_ms),
                ),
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let persisted_state = load_channel_history_state(&config.workspace_dir);
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        conversation_histories: Arc::new(Mutex::new(persisted_state.histories)),
        provider_cache: Arc::new(Mutex::new(provider_cache_seed)),
        route_overrides: Arc::new(Mutex::new(persisted_state.route_overrides)),
...
{"name":"mock_price","arguments":{"symbol":"BTC"}}
    ...
    #[tokio::test]
    async fn process_channel_message_handles_new_command_clears_history() {
        let channel_impl = Arc::new(TelegramRecordingChannel::default());
        let channel: Arc<dyn Channel> = channel_impl.clone();
        ...
        let provider = Arc::new(ModelCaptureProvider::default());
        let history_key = "telegram_alice";
        ...
        histories.insert(history_key.to_string(), initial_history);
        ...
        // Verify history exists before /new
        {
            let h = runtime_ctx.conversation_histories.lock().unwrap();
            assert!(h.contains_key(history_key));
            assert_eq!(h.get(history_key).unwrap().len(), 2);
        }
...

        {
            let sent = channel_impl.sent_messages.lock().await;
            assert_eq!(sent.len(), 1);
            assert!(sent[0].contains("Available runtime commands"));
        }

        // Clear sent messages for next part
        {
            let mut sent = channel_impl.sent_messages.lock().await;
            sent.clear();
        }
        ...
        let sent = channel_impl.sent_messages.lock().await;
        assert_eq!(sent.len(), 1);
        assert!(sent[0].contains("Session reset"));

        // Verify history is cleared after /new
        {
            let h = runtime_ctx.conversation_histories.lock().unwrap();
            assert!(!h.contains_key(history_key));
        }
