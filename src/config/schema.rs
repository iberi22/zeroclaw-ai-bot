...
// ── Integration (OpenClaw / External sync) ─────────────────────────

#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct IntegrationConfig {
    /// Sync identity and skills with OpenClaw workspace
    #[serde(default)]
    pub openclaw_sync: bool,
    /// Access and incorporate memories from external system (MEMORY.md)
    #[serde(default)]
...
pub struct AutonomyConfig {
    ...
    /// Paths that are allowed even when workspace_only=true (e.g. "E:\\scripts-python", "/home/user/projects")
    #[serde(default)]
...
/// Configuration for an additional Telegram bot instance that shares the same agent.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TelegramExtraBot {
...
pub struct TelegramConfig {
    ...
    #[serde(default)]
    pub mention_only: bool,
    /// Additional Telegram bots that share this agent's brain.
    /// Each entry requires its own bot token from \@BotFather.
    /// Add via [[channels_config.telegram.extra_bots]] in config.toml.
    #[serde(default)]
    pub extra_bots: Vec<TelegramExtraBot>,
}
