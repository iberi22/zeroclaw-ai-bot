...
impl OpenAiCompatibleProvider {
    ...
    fn sanitize_messages(messages: &[ChatMessage]) -> Vec<ChatMessage> {
        ...
        for m in messages {
            if m.role == "system" {
                ...
                if !content_str.trim().is_empty() {
                    system_contents.push(content_str);
                }
            } else {
        ...
        if !system_contents.is_empty() {
            sanitized.push(ChatMessage::system(system_contents.join("\n\n")));
        }
        sanitized.extend(other_messages);
        sanitized
    }
        ...
        messages
            ...
            .map(|message| {
                if message.role == "assistant" {
                    // For assistant messages, try to parse content as JSON for tool calls
                    let content_str = message.content_to_string();
                        ...
                        if let Some(tool_calls_value) = value.get("tool_calls") {
                                ...
                                )
                            {
                                ...
                                return NativeMessage {
                                    ...
                                    content: content.map(NativeMessageContent::String),
                                    tool_call_id: None,
                ...
                // Handle content - convert StringContent to NativeMessageContent
                let content = match &message.content {
                    crate::providers::traits::StringContent::String(s) => {
                        Some(NativeMessageContent::String(s.clone()))
                    }
                    crate::providers::traits::StringContent::Parts(parts) => {
                        let json_parts: Vec<serde_json::Value> = parts
                            .iter()
                            .map(|part| match part {
                                crate::providers::traits::MessageContentPart::Text { content } => {
                                    serde_json::json!({
                                        "type": "text",
                                        "text": content
                                    })
                                }
                                crate::providers::traits::MessageContentPart::ImageUrl { image_url } => {
                                    serde_json::json!({
                                        "type": "image_url",
                                        "image_url": {
                                            "url": image_url.url
                            ...
                            .collect();
                        Some(NativeMessageContent::Parts(json_parts))
                    }
                };
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        if let Some(system_message) = modified_messages.iter_mut().find(|m| m.role == "system") {
            ...
            if let crate::providers::traits::StringContent::String(ref mut content) =
...
impl Provider for OpenAiCompatibleProvider {
    ...
    async fn chat_with_history(
        ...
        let api_messages: Vec<Message> = Self::sanitize_messages(messages)
            .iter()
            .map(|m| Message {
                role: m.role.clone(),
            ...
            if status == reqwest::StatusCode::NOT_FOUND && self.supports_responses_fallback {
                ...
                if let Some(user_msg) = last_user {
                    return self
                        .chat_via_responses(
                            credential,
                            system.map(|m| m.content_as_str()),
                            &user_msg.content_to_string(),
                            model,
                        )
                        .await
                        .map_err(|responses_err| {
    ...
    async fn chat(
        ...
        let native_request = NativeChatRequest {
            model: model.to_string(),
            messages: Self::convert_messages_for_native(&Self::sanitize_messages(request.messages)),
            temperature,
            stream: Some(false),
