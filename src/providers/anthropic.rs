use crate::providers::traits::{
    ...
    Provider, StringContent, ToolCall as ProviderToolCall,
};
...
impl AnthropicProvider {
    ...
    fn convert_messages(messages: &[ChatMessage]) -> (Option<SystemPrompt>, Vec<NativeMessage>) {
        let mut system_text: Option<StringContent> = None;
        ...
        for msg in messages {
            match msg.role.as_str() {
                ...
                "assistant" => {
                    if let Some(blocks) =
                        Self::parse_assistant_tool_call_message(&msg.content.to_string())
                    {
                ...
                "tool" => {
                    if let Some(tool_result) =
                        Self::parse_tool_result_message(&msg.content.to_string())
                    {
        ...
        let system_prompt = system_text.map(|text| {
            ...
            if Self::should_cache_system(&text_str) {
