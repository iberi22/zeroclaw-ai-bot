...
/// Image URL specification for multimodal content.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ImageUrl {
    /// The URL of the image (http/https or data URL with base64).
    pub url: String,
    /// Detail level: "low", "high", or "auto". Defaults to "auto".
    #[serde(skip_serializing_if = "Option::is_none")]
    pub detail: Option<String>,
}

impl ImageUrl {
    /// Create a new image URL from a data URL (base64).
    ...
    /// Create a new image URL from a HTTP/HTTPS URL.
    pub fn from_url(url: String) -> Self {
        Self { url, detail: None }
    }

    /// Set the detail level for the image.
...
pub struct ChatMessage {
    pub role: String,
    /// Content can be either a simple string (for text-only messages)
    /// or a vector of content parts (for multimodal messages).
    #[serde(skip_serializing_if = "StringContent::is_string_content")]
    pub content: StringContent,
}

impl ChatMessage {
    pub fn system(content: impl Into<StringContent>) -> Self {
        Self {
            role: "system".into(),
    ...
    pub fn user(content: impl Into<StringContent>) -> Self {
        Self {
            role: "user".into(),
            content: content.into(),
        }
    ...
    pub fn assistant(content: impl Into<StringContent>) -> Self {
        Self {
            role: "assistant".into(),
            content: content.into(),
        }
    ...
    pub fn tool(content: impl Into<StringContent>) -> Self {
        Self {
            role: "tool".into(),
            content: content.into(),
        }
...
/// Content can be either a plain string or a vector of multimodal parts.
/// This type handles both cases for serialization/deserialization.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(untagged)]
pub enum StringContent {
    /// Plain text content (simple string).
    String(String),
...
impl StringContent {
    /// Check if this is string content (for skip_serializing_if).
    fn is_string_content(&self) -> bool {
        matches!(self, StringContent::String(_))
    }
}

impl Deref for StringContent {
    type Target = str;
...
impl AsRef<str> for StringContent {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
...
impl From<&String> for StringContent {
    fn from(s: &String) -> Self {
        StringContent::String(s.clone())
    }
}

impl From<&str> for StringContent {
    fn from(s: &str) -> Self {
        StringContent::String(s.to_string())
    }
}

impl From<Vec<MessageContentPart>> for StringContent {
    fn from(parts: Vec<MessageContentPart>) -> Self {
        StringContent::Parts(parts)
    }
}

impl PartialEq<&str> for StringContent {
...
impl PartialEq<String> for StringContent {
    fn eq(&self, other: &String) -> bool {
        self.as_str() == other.as_str()
    }
}

impl ToString for StringContent {
...
impl StringContent {
    /// Get the content as a string, converting from parts if necessary.
    /// For multimodal content, concatenates text parts.
    pub fn as_str(&self) -> &str {
        match self {
            StringContent::String(s) => s.as_str(),
            StringContent::Parts(parts) => {
                static EMPTY: &str = "";
                for part in parts {
                    if let MessageContentPart::Text { content } = part {
    ...
    /// Check if the content is text-only (not multimodal).
    pub fn is_text_only(&self) -> bool {
        matches!(self, StringContent::String(_))
    }

    /// Extract the string content, returning empty string for multimodal content.
    pub fn to_string_lossy(&self) -> String {
        match self {
            StringContent::String(s) => s.clone(),
            StringContent::Parts(parts) => {
...
impl ChatMessage {
    ...
    /// Get the content as a owned string.
    pub fn content_to_string(&self) -> String {
        self.content.to_string_lossy()
    }

    /// Create a user message with text and optional images.
    /// This enables multimodal input for vision-capable models.
    pub fn user_with_images(text: impl Into<String>, images: Vec<ImageUrl>) -> Self {
        let mut parts: Vec<MessageContentPart> = vec![MessageContentPart::Text {
            content: text.into(),
        }];

        for image in images {
            parts.push(MessageContentPart::ImageUrl { image_url: image });
        }

        Self {
            role: "user".into(),
            content: StringContent::Parts(parts),
        }
    }

    /// Create a user message from a data URL (base64 encoded image).
    pub fn user_with_base64_image(text: impl Into<String>, image_data_url: String) -> Self {
        Self::user_with_images(text, vec![ImageUrl::from_data_url(image_data_url)])
    }
}
    ...
    async fn chat(
        ...
        if let Some(tools) = request.tools {
            if !tools.is_empty() && !self.supports_native_tools() {
                ...
                {
                    // Only inject into text-only content
                    if let StringContent::String(ref mut content) = system_message.content {
                        if !content.is_empty() {
                            content.push_str("\n\n");
                        }
